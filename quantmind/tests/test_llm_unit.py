"""QuantMind v2 — LLM client unit tests (fully mocked; no real API calls).
...
from quantmind.llm import GeminiClient, LLMError, LLMStructuredOutputError
from quantmind.schemas import MemoryContext
from quantmind.utils import gemini_limiter
...
def _make_client(mocker, mock_model):
    ...
    mocker.patch(
        "quantmind.llm.client.create_llm_client", return_value=fake_factory_client
    )
    return GeminiClient()
