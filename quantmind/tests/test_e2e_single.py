"""QuantMind v2 — end-to-end single-ticker demo test (the primary portfolio demo).

Marked ``live``. Run: pytest quantmind/tests/test_e2e_single.py -v -m live -s
...
async def test_single_ticker_full_pipeline():
    ...
    start = time.time()
    ta = await runner.analyze_ticker("AAPL")
    ...
    # --- DB persistence ---
    analyses = await runner.db.get_recent_analyses("AAPL", days=1)
    ...
    async with aiosqlite.connect(runner.db.db_path) as conn:
        async with conn.execute(
            "SELECT DISTINCT agent_name FROM agent_runs WHERE ticker = 'AAPL'"
        ) as cur:
            agent_names = {row[0] for row in await cur.fetchall()}
