"""QuantMind v2 — live LLM integration test (hits the real Gemini API).

Marked ``live`` and DESELECTED by default (pyproject ``addopts = -m 'not live'``).
Run explicitly:  pytest quantmind/tests/test_llm_live.py -v -m live -s
...
from quantmind.llm import GeminiClient, news_analyze_prompt, news_select_prompt
from quantmind.schemas import Article, MemoryContext, NewsAnalysis
...
async def test_full_llm_pipeline():
    ...
    response = await client.generate_text("Respond with exactly the word: QUANTMIND_OK")
    assert "QUANTMIND_OK" in response.upper()
