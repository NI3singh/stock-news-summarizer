"""QuantMind — MCP server (Phase C).

Exposes the QuantMind pipeline to MCP clients (Claude Desktop, IDEs, etc.) via
FastMCP. Lives under ``quantmind/integrations/`` (NOT ``quantmind/mcp/``) on
purpose: FastMCP depends on the installed ``mcp`` SDK, and a top-level
``quantmind/mcp/`` package would shadow it when the app runs as
``python quantmind/main.py`` (``quantmind/`` on ``sys.path[0]``).
...
mcp = FastMCP(
    name="QuantMind",
    instructions="""
    You are connected to QuantMind — a multi-agent quantitative news intelligence system.
...
async def get_system_status() -> dict:
    ...
    return {
        ...
        "system_note": "QuantMind v2 — Multi-agent async pipeline. 4 agents: memory, news, quant, orchestrator.",
    }
