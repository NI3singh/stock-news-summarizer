"""QuantMind — Telegram bot (Phase B).

Lives under ``quantmind/integrations/`` (NOT ``quantmind/telegram/``) on purpose:
``python-telegram-bot`` installs a top-level package named ``telegram``, and when
the app is run as ``python quantmind/main.py`` the ``quantmind/`` directory is on
``sys.path[0]`` — a ``quantmind/telegram/`` folder there would shadow the real
...
class QuantMindBot:
    """Telegram front-end over the QuantMind pipeline."""

    def __init__(self, runner: PipelineRunner) -> None:
        self.runner = runner
        self.app = Application.builder().token(settings.telegram_bot_token).build()
        self._register_handlers()
        logger.info("QuantMindBot initialized")
    ...
    async def cmd_start(self, update, context) -> None:
        await update.message.reply_text(
            "🤖 *QuantMind AI is connected!*\n\n"
            "I'm your personal stock market intelligence assistant.\n\n"
            "*What I can do:*\n"
            "• Analyze any stock with multi-agent AI\n"
            "• Alert you when sentiment shifts significantly\n"
            "• Give you daily morning briefings\n\n"
    ...
    async def cmd_help(self, update, context) -> None:
        await update.message.reply_text(
            "*QuantMind Commands*\n\n"
            "`/analyze TICKER` — Run full AI analysis (e.g. /analyze AAPL)\n"
            "`/add TICKER` — Add ticker to your watchlist\n"
            "`/remove TICKER` — Remove from watchlist\n"
            "`/watchlist` — Show all active tickers with latest sentiment\n"
            "`/status` — System health and last refresh time\n"
    ...
    def _format_analysis(self, ta: TickerAnalysis) -> str:
        ...
        return (
            ...
            f"_Analyzed by QuantMind's 4 AI agents_"
        )
    ...
    async def send_daily_summary(self) -> None:
        """Compose and push a one-line-per-ticker daily watchlist summary."""
        tickers = await self.runner.db.get_active_tickers()
        if not tickers:
            return
        lines = ["*📊 QuantMind Daily Summary*", ""]
        for ticker in tickers:
