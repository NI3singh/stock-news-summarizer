"""StockStalker v2 — LLM module tests (provider-agnostic factory).
...
from stockstalker.llm import BaseLLMClient, create_llm_client
from stockstalker.llm.anthropic_client import AnthropicClient
from stockstalker.llm.base_client import normalize_content
from stockstalker.llm.capabilities import get_capabilities
...
from stockstalker.schemas import TechnicalSignals

_PROJECT_ROOT = Path(__file__).resolve().parents[2]


def test_lazy_imports_no_sdk():
    """`import stockstalker.llm` must NOT pull any LangChain provider SDK (gotcha #8).

    Run in a fresh subprocess so other tests loading LangChain can't pollute it.
    """
    code = (
        "import sys, stockstalker.llm; "
...
def test_build_llms_from_config():
    ...
    from stockstalker.llm.builder import build_llms, get_provider_kwargs
