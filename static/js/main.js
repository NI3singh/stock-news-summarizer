...
//     // Show loading immediately for current ticker
//     if (currentSelectedTicker === symbol) {
//         document.getElementById('summaryContainer').classList.add('hidden');
//         document.getElementById('welcomeMessage').classList.add('hidden');
//         document.getElementById('loadingState').classList.remove('hidden');
...
//         if (data.success) {
//             // Wait 5 seconds before starting to poll (give backend time to start processing)
...
//                     if (summaryData.success && summaryData.latest_summary && summaryData.latest_summary.created_at) {
//                         // Convert created_at string to timestamp
//                         const summaryCreatedTime = new Date(summaryData.latest_summary.created_at).getTime();
                        
//                         // Only accept if summary was created AFTER we clicked refresh
//                         if (summaryCreatedTime > refreshStartTime) {
//                             // SUCCESS - New fresh summary!
//                             clearInterval(checkCompletion);
//                             isProcessing = false;
...
//                         clearInterval(checkCompletion);
//                         isProcessing = false;
//                         showNotification('Processing complete.', 'info');
...
//     isProcessing = true;
//     processingTicker = symbol; // mark this ticker as processing in UI
//     showNotification(`Starting refresh for ${symbol}...`, 'info');

//     // If the symbol is currently selected, show loading state
//     if (currentSelectedTicker === symbol) {
//         document.getElementById('summaryContainer').classList.add('hidden');
//         document.getElementById('welcomeMessage').classList.add('hidden');
//         document.getElementById('loadingState').classList.remove('hidden');
...
//     // Update sidebar to show the "Processing" badge
//     try {
//         const tickers = await fetch('/api/tickers').then(r => r.json()).then(d => d.tickers);
//         displayTickers(tickers);
//     } catch (e) {
...
//     // --- NEW: fetch the baseline summary created_at so we can detect a truly new summary ---
//     let baselineCreatedAt = null;
//     try {
//         const baselineResp = await fetch(`/api/summary/${symbol}`);
//         const baselineJson = await baselineResp.json();
//         if (baselineJson && baselineJson.success && baselineJson.latest_summary && baselineJson.latest_summary.created_at) {
//             baselineCreatedAt = baselineJson.latest_summary.created_at;
//         }
//     } catch (err) {
...
//         if (data.success) {
//             // Wait a short while to let backend job start
...
//                     if (summaryData.success && summaryData.latest_summary) {
//                         const latest = summaryData.latest_summary;
//                         const latestCreated = latest.created_at || latest.updated_at || null;

//                         // Accept the summary if:
//                         // 1) created_at differs from baseline (server indicates a new summary), OR
//                         // 2) if baseline is missing, accept if created_at is present and reasonably after our refreshStartTime (allowing some clock skew)
//                         let isNew = false;
//                         if (baselineCreatedAt && latestCreated && latestCreated !== baselineCreatedAt) {
//                             isNew = true;
//                         } else if (!baselineCreatedAt && latestCreated) {
//                             const parsed = Date.parse(latestCreated);
//                             if (!isNaN(parsed) && parsed > (refreshStartTime - 120000)) { // allow 2m skew backward
//                                 isNew = true;
//                             }
//                         }
...
//                                 displaySummary(summaryData);
//                             } else {
//                                 // If user has selected a different ticker while processing, don't force view change.
//                                 // But show notification with instruction.
//                                 showNotification(`${symbol} updated — click it to view.`, 'success');
//                             }
//                             showNotification(`${symbol} updated successfully!`, 'success');
//                             return;
//                         }
...
//                     if (attempts >= maxAttempts) {
//                         // Timeout - stop polling and show whatever we have (but do not display an old summary automatically)
//                         clearInterval(checkCompletion);
//                         isProcessing = false;
//                         processingTicker = null;

//                         // Update UI - hide loading if this ticker is selected but still no new summary
//                         if (currentSelectedTicker === symbol) {
//                             document.getElementById('loadingState').classList.add('hidden');
//                             // Try to re-fetch summary once more (may still be old)
//                             try {
//                                 const tryResp = await fetch(`/api/summary/${symbol}`);
//                                 const tryJson = await tryResp.json();
//                                 if (tryJson.success && tryJson.latest_summary) {
//                                     displaySummary(tryJson);
//                                 } else {
//                                     document.getElementById('summaryContainer').classList.remove('hidden');
//                                 }
//                             } catch (e) {
...
//                         showNotification('Processing timed out. Please try again later.', 'info');

//                         // finally refresh sidebar
//                         try {
//                             const tickers = await fetch('/api/tickers').then(r => r.json()).then(d => d.tickers);
//                             displayTickers(tickers);
//                         } catch (e) {}
...
//                 } catch (error) {
//                     console.error('Error checking completion:', error);
//                     // continue polling; occasional network hiccups are fine
//                 }
//             }, 3000);
//         } else {
//             isProcessing = false;
...
// Refresh specific ticker (robust for new tickers with no baseline)
// Replace your current refreshTicker with this robust, cache-busting version
// async function refreshTicker(symbol) {
//     if (isProcessing) {
//         showNotification('Already processing, please wait...', 'warning');
//         return;
...
//     // Snapshot baseline (cache-bypassed)
//     let baselineCreatedAt = null;
//     let baselineSummaryText = null;
//     try {
//         const baselineResp = await fetch(`/api/summary/${symbol}?cb=${Date.now()}`, { cache: 'no-store' });
//         const baselineJson = await baselineResp.json();
//         if (baselineJson && baselineJson.success && baselineJson.latest_summary) {
//             baselineCreatedAt = baselineJson.latest_summary.created_at || baselineJson.latest_summary.updated_at || null;
//             baselineSummaryText = (baselineJson.latest_summary.summary_text || '').trim() || null;
//         }
//     } catch (err) {
//         // ignore - baseline stays null
//     }
...
//         if (data.success) {
//             // Wait briefly so backend has time to start (small delay)
//             await new Promise(resolve => setTimeout(resolve, 3000));
            
//             // Poll for completion - check every 3 seconds
//             let attempts = 0;
//             const maxAttempts = 50; // increase to be safer
//             const checkCompletion = setInterval(async () => {
//                 attempts++;
                
//                 try {
//                     // Cache-busting fetch each time
//                     const summaryResponse = await fetch(`/api/summary/${symbol}?cb=${Date.now()}`, { cache: 'no-store' });
//                     const summaryData = await summaryResponse.json();

//                     // DEBUG: print what server returns so we can see why it's accepted/rejected
//                     try {
//                         console.debug(`[refreshTicker:${symbol}] poll #${attempts}`, {
//                             success: !!(summaryData && summaryData.success),
//                             has_latest_summary: !!(summaryData && summaryData.latest_summary),
//                             created_at: summaryData?.latest_summary?.created_at,
//                             text_len: (summaryData?.latest_summary?.summary_text || '').length
//                         });
//                     } catch (e) {}

//                     if (summaryData && summaryData.success && summaryData.latest_summary) {
//                         const latest = summaryData.latest_summary;
//                         const latestCreated = latest.created_at || latest.updated_at || null;
//                         const latestText = (latest.summary_text || '').trim();

//                         // Acceptance rules:
//                         // 1) created_at changed from baseline
//                         // 2) summary_text differs from baseline summary_text
//                         // 3) baseline had no summary_text and latestText is non-empty and reasonably long
//                         let isNew = false;

//                         if (baselineCreatedAt && latestCreated && latestCreated !== baselineCreatedAt) {
//                             isNew = true;
//                             console.debug(`[refreshTicker:${symbol}] accepted: created_at changed`);
//                         }

//                         if (!isNew && baselineSummaryText && latestText && latestText !== baselineSummaryText) {
//                             isNew = true;
//                             console.debug(`[refreshTicker:${symbol}] accepted: summary_text changed`);
//                         }

//                         if (!isNew && !baselineSummaryText && latestText && latestText.length > 30) {
//                             // If no baseline text, accept any substantial text (allow clock skew)
//                             if (!latestCreated) {
//                                 isNew = true;
//                                 console.debug(`[refreshTicker:${symbol}] accepted: new text (no created_at)`);
//                             } else {
//                                 const parsed = Date.parse(latestCreated);
//                                 if (isNaN(parsed) || parsed > (refreshStartTime - 120000)) {
//                                     isNew = true;
//                                     console.debug(`[refreshTicker:${symbol}] accepted: new text with recent created_at`);
//                                 }
//                             }
//                         }

//                         // Strong fallback: if latestText is long and attempts > small threshold, accept it
//                         if (!isNew && latestText && latestText.length > 200 && attempts > 3) {
//                             isNew = true;
//                             console.debug(`[refreshTicker:${symbol}] accepted: long text fallback`);
//                         }
...
//                     if (attempts >= maxAttempts) {
//                         // Timeout - do one final forced fetch and show whatever we have
//                         clearInterval(checkCompletion);
//                         isProcessing = false;
//                         processingTicker = null;
...
//                             console.debug(`[refreshTicker:${symbol}] final fetch before timeout`, {
//                                 success: !!(finalJson && finalJson.success),
//                                 created_at: finalJson?.latest_summary?.created_at,
//                                 text_len: (finalJson?.latest_summary?.summary_text || '').length
//                             });

//                             if (finalJson && finalJson.success && finalJson.latest_summary) {
//                                 if (currentSelectedTicker === symbol) {
//                                     displaySummary(finalJson);
//                                 } else {
//                                     showNotification(`${symbol} updated — click it to view.`, 'info');
//                                 }
//                             } else {
//                                 if (currentSelectedTicker === symbol) {
//                                     document.getElementById('loadingState').classList.add('hidden');
...
//                         showNotification('Processing finished (or timed out). See latest if available.', 'info');
...
// Refresh all tickers (robust, shows loading per ticker and polls each summary)
// Refresh all tickers with professional UX: per-ticker badges, per-ticker notifications,
// do NOT hide the current summary on start, and auto-load completed tickers only if user didn't interact.
// Replace existing refreshAll with this improved version
async function refreshAll() {
    if (!confirm('Refresh all tickers? This may take a few minutes.')) return;
    if (isProcessing) {
